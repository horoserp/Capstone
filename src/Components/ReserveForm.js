...
function ReserveForm(state, stateChange) {
  ...
  const event = ["--Select--", "Birthday", "Anniversary", "Engagement"];
  ...
  const dateError = () => {
    if (dateValidating && date === "") {
      return <div className="col-1 error">Date is required</div>;
  ...
  const timeError = () => {
    if (timeValidating && (time === "" || time === "--Select a Time--")) {
      return <div className="col-2 error">Please pick a time</div>;
    }
  ...
  const guestError = () => {
    if (guestValidating && numOfGuests === "") {
      ...
      return <div className="col-1 error">You must have at least 1 guest</div>;
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="grid">
        <div>
          <label htmlFor="res-date" className="gap">
            {dateError()}
          </label>
          <div>
            <input
        ...
        <div>
          <label htmlFor="guests" className="gap">
            {guestError()}
          </label>
          <div>
            <input
